...
Mock.mock('/users/active', 'get', () => {
  ...
  usersObj = comments.reduce((accumulator, currentValue) => {
    accumulator[currentValue.uname] = accumulator[currentValue] || {};
...
Mock.mock('/articles/hot', 'post', options => {
  // 将评论最少的文章排在前面
  let filtersArticles = store.getters.getArticlesByFilter('noreply');
  // 将评论最多的文章排在前面
  let articles = filtersArticles.reverse();
  // 取 7 天内评论最多的文章
